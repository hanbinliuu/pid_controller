...
class PIDController:
    ...
    def __init__(self, pb=100, ti=0, td=0, dt=1, u_min=0, u_max=100, mode="standard", system_mode="standard"):
        ...
        self.Kp = 100 / pb if pb != 0 else 0  # 比例增益
        self.Ti = ti  # 积分时间
        self.Td = td  # 微分时间
        ...
        self.target_ti = ti
        self.target_td = td
        self.target_Kp = 100 / pb if pb != 0 else 0
        self.target_Ti = ti
        self.target_Td = td
...
class Visualizer:
    ...
    def init_real_time_fig(self, t, u, setpoint, system_params, pid_params, response_data, true_params, disturbances,
        ...
        self.plot_data = {
            ...
            'lines': (temp_line, valve_line, error_line, pb_line, ti_line, td_line, new_pid_line),
            'text': params_text,
...
目标温度: {Config.TARGET_TEMPERATURE}℃ | 初始温度: {Config.INIT_TEMPERATURE}℃"""
    ...
    def update_plots(self, time_data, temp_data, valve_data, error_data, pb_data, ti_data, td_data,
                     update_lines, new_pid_time_data=None, new_pid_temp_data=None):
        """实时更新绘图数据"""
        (temp_line, valve_line, error_line, pb_line, ti_line, td_line, new_pid_line) = self.plot_data['lines']
...
class ControlMonitor:
    ...
    def run(self):
        ...
        if using_json_data:
            ...
            if len(pb_data) > 0 and len(ti_data) > 0 and len(td_data) > 0:
                ...
                init_pb, init_ti, init_td = self.identifier.lambda_tuning(K, T, L, mode=system_mode)
        else:
            # 生成仿真数据（如未提供）
        ...
        self._run_real_time_monitor(u, setpoint=Config.TARGET_TEMPERATURE, true_params=initial_true_params,
                                    ...
                                    pb_data=pb_data, ti_data=ti_data, td_data=td_data)
    ...
    def _run_real_time_monitor(self, u, setpoint, true_params, identified_params, response_data, disturbances,
        ...
        # 初始化图表
        fig, plot_data = self.visualizer.init_real_time_fig(initial_time, u, setpoint, identified_params,
        ...
        temp_line, valve_line, error_line, pb_line, ti_line, td_line, new_pid_line = plot_data['lines']
        ...
        try:
            while True:  # 无限运行主循环
                ...
                if stabilization_time is None and state_analyzer.is_stable(temp_data, setpoint):
                    ...
                    tuning_enabled = True
                    initial_valid_params = (pid.pb, pid.ti, pid.td)  # 记录初始有效参数
                    ...
                    if is_disturbance:
                        ...
                        if current_disturbance:
                            ...
                            if not current_disturbance['updated'] and (time - current_disturbance["time"]) > 50:
                                ...
                                new_pb, new_ti, new_td = self.identifier.lambda_tuning(K, T, L, lambda_val,
                                                                                       mode=system_mode)
                ...
                if tuning_enabled and not using_json_data:
                    ...
                    if all(recovery_status.get(d["type"], True) for d in disturbances):
                        # 随机决定是否生成新扰动（每2000步有50%概率生成新扰动）
                        if time % 1500 == 0 and np.random.random() > 0.4:
                            # 生成新的扰动
                            new_disturbance_type = np.random.choice(Config.DISTURBANCE_TYPES)
                            new_disturbance = {
                                "time": time + 100,  # 100秒后开始新的扰动
                ...
                if step % 50 == 0:
                    ...
                    print(
                        f"时间: {time:.0f}s | 系统模式: {Config.MODES[system_mode]} | PID模式: {Config.PID_MODES[pid_mode]} | 数据源: {'JSON' if using_json_data else '仿真'} | 实际响应温度: {current_temp:.1f}℃ | 误差: {error:.2f}℃ | "
                        f"阀门: {valve_opening:.1f}% | 状态: {stable_status} | 更新: {params_update_count}次 | {tuning_phase_status} | 扰动状态: {recovery_text}")
                ...
                if step % Config.PLOT_REFRESH_INTERVAL == 0:
                    self.visualizer.update_plots(time_data, temp_data, valve_data, error_data,
                                                 ...
                                                 new_pid_time_data, new_pid_temp_data)
            ...
            print("监控手动终止")
            print(
                f"当前实际响应温度：{current_temp:.1f}℃ | 误差：{error:.2f}℃")
            print(f"初始有效参数 → 当前参数：")

...
class Config:
    ...
    # 稳态判断参数（严格化）
    STABILIZATION_THRESHOLD = 0.8       # 温度波动允许阈值(℃)，更严格
    STABILIZATION_WINDOW = 40           # 稳态判断窗口大小，增加样本量

    # 扰动配置（增强扰动效果）
    DISTURBANCES = [
        {"time": 600, "type": "overshoot", "duration": 50, "amplitude": 4, "description": "超调量大"},
        {"time": 1200, "type": "steady_error", "duration": 180, "amplitude": 2, "description": "稳态误差变大"},
        {"time": 2000, "type": "slow_recovery", "duration": 120, "amplitude": 3, "description": "恢复时间延长"},
        {"time": 2700, "type": "steady_error", "duration": 90, "amplitude": 5, "description": "稳态误差变大"}
    ]

    # 系统老化与辨识参数（强化老化影响）
    AGING_START_TIME = 300          # 老化开始时间(s)，稍晚于初始稳态
    IDENTIFY_WINDOW = 200           # 参数辨识窗口大小(点数)，增加数据量
    PARAM_UPDATE_SMOOTH_FACTOR = 0.2  # 更平缓的参数更新
...
class TemperatureSystem:
    ...
    def apply_aging(self, time):
        """增强老化对参数的影响，确保旧PID参数会失效"""
        if time < Config.AGING_START_TIME:
            return

        # 老化因子（最大变化提升至80%，加速参数失效）
        aging_factor = min(0.002 * (time - Config.AGING_START_TIME), 0.8)
        self.K = self.initial_K * (1 - aging_factor * 0.4)  # 增益降低更明显
        self.T = self.initial_T * (1 + aging_factor * 0.8)  # 时间常数增大更明显
        self.L = self.initial_L * (1 + aging_factor * 0.9)  # 滞后时间增加更明显
        ...
        for disturbance in Config.DISTURBANCES:
            ...
            if start <= time < end:
                if disturbance["type"] == "overshoot":
                    ...
                    # 稳态误差：持续偏移且带波动
                    turbulent = np.sin((time - start) * 0.1) * disturbance["amplitude"] * 0.3  # 修复缩进错误
...
class PIDController:
    ...
    def __init__(self, Kp=0, Ti=0, Td=0, dt=1, u_min=0, u_max=100):
        ...
        self.integral = 0.0
        self.derivative = 0.0
        self.smoothing_factor = Config.PARAM_UPDATE_SMOOTH_FACTOR  # 更平缓的更新
...
def identify_fopdt_least_squares(t, y, u):
    """用最小二乘法辨识FOPDT模型参数"""
    y0 = np.mean(y[-30:]) if len(y) > 30 else np.mean(y)  # 更稳定的初始值估计
    max_u = np.max(u)
    max_y = np.max(y)
...
def detect_instability(temp_data, setpoint, valve_data, window=80):
    ...
    if valve_changes > 180 and steady_error > 2.0:  # 阀门频繁调整但误差大
        return True, "恢复时间延长"
...
def init_real_time_fig(t, u, setpoint, system_params, pid_params, response_data):
    ...
    # 3. 温度控制曲线（含扰动标记）
    ax2 = fig.add_subplot(gs[1:3, :])  # 更大的绘图区域
    ...
    update_lines = []  # 参数更新线
    update_marker, = ax2.plot([], [], 'bo', markersize=6, label='参数更新点')  # 参数更新标记
    ...
    ax3.set_ylim(0, 100)
    for d in Config.DISTURBANCES:
        ax3.axvspan(d["time"], d["time"] + d["duration"], color=disturbance_colors[d["type"]],
                    alpha=0.2)
    ...
    for d in Config.DISTURBANCES:
        ax4.axvspan(d["time"], d["time"] + d["duration"], color=disturbance_colors[d["type"]],
                    alpha=0.2)
    ...
    ax5.set_ylim(0, 6)
    for d in Config.DISTURBANCES:
        ax5.axvspan(d["time"], d["time"] + d["duration"], color=disturbance_colors[d["type"]],
                    alpha=0.2)
...
def run_real_time_monitor(t, u, setpoint, true_params, identified_params, response_data):
    ...
    stabilization_line, update_lines, update_marker = plot_data['markers']
    ...
    try:
        for i, time in enumerate(t):
            ...
            if current_disturbance and tuning_enabled:
                # 扰动开始后10s内强制更新参数（避免初期波动误判）
                if not current_disturbance['updated'] and (time - current_disturbance["time"]) > 20:
                    print(f"\n⚠️ 检测到[{current_disturbance['description']}]，强制重新整定参数...")

                    # 窗口数据提取（扰动前30s+扰动后10s）
            ...
            if i % Config.PLOT_REFRESH_INTERVAL == 0:
                ...
                update_marker.set_data(update_times, [setpoint + 8] * len(update_times))  # 固定在目标温度上方

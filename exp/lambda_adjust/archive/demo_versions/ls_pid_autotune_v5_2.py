...
class Config:
    ...
    DISTURBANCE_TYPES = [
        ...
        {"type": "flow_noise", "description": "流量噪声增大"},  # 新增：流量噪声
        {"type": "flow_stiction", "description": "流量阀门卡涩"}  # 新增：流量阀门卡涩
    ]
...
class DataHandler:
    ...
    def read_csv_data(self, file_path):
        """读取CSV格式的数据 - 与自动生成数据格式相同"""
        try:
            df = pd.read_csv(file_path)
            required_cols = ["时间(s)", "动态响应值"]
            if not all(col in df.columns for col in required_cols):
                raise ValueError(f"CSV文件必须包含列：{required_cols}")

            df = df.dropna(subset=required_cols)
            t = df["时间(s)"].values
            data = df["动态响应值"].values
            ...
            # 应用低通滤波到数据
            data = self.apply_low_pass_filter(data)
            self.logger.info(
                f"对数据应用了低通滤波 (降噪强度: {self.noise_reduction_level}, 截止频率: {Config.NOISE_REDUCTION_LEVELS[self.noise_reduction_level]['cutoff_freq']})")
    ...
    def choose_mode(self):
        ...
        while True:
            choice = input("请选择运行模式（1-4，直接回车选1）：").strip() or "1"
            if choice == "1":
                return "standard"
            elif choice == "2":
    ...
    def choose_control_type(self):
        ...
        while True:
            choice = input("请选择控制类型（1-2，直接回车选1）：").strip() or "1"
            if choice == "1":
                return "temperature"
            elif choice == "2":
                return "flow"
...
class DisturbanceHandler:
    ...
    def __init__(self, mode="standard"):
        ...
        self.disturbance_functions = {
            ...
            "flow_noise": self._apply_flow_noise,  # 新增
            "flow_stiction": self._apply_flow_stiction  # 新增
        }
    ...
    def _apply_overshoot(self, time, current_temp, valve_opening, amplitude, duration, start_time, system_state):
        ...
        if self.mode == "anti_disturbance":
            ...
            # 抗噪声模式下扰动幅度减小
            amplitude *= 0.8
        elif self.mode == "flow_control":
            amplitude *= 0.5  # 流量模式下扰动影响较小
...
class FlowSystem:
    ...
    def __init__(self, K=0.6, T=1.0, noise_level=0.5, mode="flow_control"):
        self.mode = mode
        self.K = K  # 增益
    ...
    def simulate_with_input(self, t, u, disturbances):
        """基于输入信号仿真流量响应"""
        flow = np.ones_like(t) * Config.INIT_FLOW
        self.last_flow = Config.INIT_FLOW

        for i in range(len(t)):
            dt = t[i] - t[i - 1] if i > 0 else 1
            steady_state = u[i] * self.K
            self.last_flow += (steady_state - self.last_flow) / self.T * dt
    ...
    def update(self, valve_opening, time, dt=1, disturbances=None):
        ...
        # 阀门开度到流量的映射，考虑饱和和非线性
        # 阀门开度0-100%，对应流量0-最大流量
        max_flow = self.K * 100  # 最大流量
        steady_state = min(valve_opening * self.K, max_flow)  # 限制最大流量
        ...
        # 限制流量范围
        output_flow = max(0, output_flow)  # 流量不能为负
        return output_flow
...
class FlowSetpointManager:
    """流量设定值管理器：实现设定值斜坡功能"""

    def __init__(self, ramp_rate=0.8):
        self.current_sp = 0.0
        self.ramp_rate = ramp_rate

    def update(self, target_sp, dt):
        delta = target_sp - self.current_sp
        max_delta = self.ramp_rate * dt
        if abs(delta) <= max_delta:
            self.current_sp = target_sp
        else:
            self.current_sp += np.sign(delta) * max_delta
...
class PIDController:
    ...
    def __init__(self, Kp=0, Ti=0, Td=0, dt=1, u_min=0, u_max=100, mode="standard", system_mode="standard"):
        ...
        self.max_output_rate = 0.5  # 每秒最多变化0.5%（流量控制更平滑）

        # 优化：测量值滤波器（用于流量控制）
        self.process_var_filter = LowPassFilter(alpha=0.15 if system_mode == "flow_control" else 0.25)

    def compute(self, setpoint, process_var):
        """计算PID输出（含抗积分饱和）"""
        # 优化：先对测量值滤波（减少噪声影响）
        if self.system_mode == "flow_control":
            process_var = self.process_var_filter.update(process_var)
        ...
        elif self.system_mode == "flow_control":
            # 流量控制模式：关闭微分项，减少噪声影响
            adjusted_Td = 0.0
        else:
            adjusted_Td = self.Td
        ...
        # 优化：输出变化率限制（流量控制更平滑）
        if self.system_mode == "flow_control":
            max_delta = self.max_output_rate * self.dt
...
class StateAnalyzer:
    ...
    def is_stable(self, data, setpoint):
        """判断系统是否稳定在目标值附近"""
        if len(data) < Config.STABILIZATION_WINDOW:
            return False
        recent_data = np.array(data[-Config.STABILIZATION_WINDOW:])
        ...
        # 均值需接近目标值（±0.5）
        mean_data = np.mean(recent_data)
        if not (setpoint - 0.5 <= mean_data <= setpoint + 0.5):
            return False
        # 波动判断
    ...
    def detect_instability_optimized(self, data, setpoint, valve_data, time_data):
        """优化的扰动检测算法 - 增强对轻微扰动的检测能力"""
        if len(data) < Config.DETECTION_WINDOW:
            return False, ""

        # 获取当前窗口的数据
        recent_data = np.array(data[-Config.DETECTION_WINDOW:])
        ...
        mean_abs_deriv = np.mean(np.abs(data_deriv))
        ...
        # 数据模式变化检测
        elif data_pattern_change > 0.5:
            is_disturbance = True
            disturbance_type = f"数据模式变化({data_pattern_change:.2f})"
...
class Visualizer:
    ...
    def init_real_time_fig(self, t, u, setpoint, system_params, pid_params, response_data, true_params, disturbances,
                           pid_mode, system_mode, control_type):
        """初始化实时监控图表"""
        ...
        if control_type == "flow":
            identified_y = SystemIdentifier.first_order_model([K, T], t, u, Config.INIT_FLOW)
        else:
        ...
        ax1.set_title(
            f'系统响应与辨识模型对比 (PID模式: {Config.PID_MODES[pid_mode]}, 系统模式: {Config.MODES[system_mode]}, 控制类型: {control_type})')
        ax1.set_xlabel('时间 (s)')
        ax1.set_ylabel(f'响应值 ({"流量" if control_type == "flow" else "温度"})')
        ax1.legend()
        ...
        if control_type == "flow":
            ax2.set_ylim(Config.INIT_FLOW - 15, setpoint + 25)
        else:
            ax2.set_ylim(Config.INIT_TEMPERATURE - 15, setpoint + 25)
        ax2.axhline(setpoint, color='r', linestyle='--', label='设定值', linewidth=1.5)
        if control_type == "flow":
            ax2.axhline(Config.INIT_FLOW, color='orange', linestyle='-.',
                        label=f'初始值 {Config.INIT_FLOW}', linewidth=1.5)
        else:
        ...
        disturbance_colors = {
            ...
            "flow_noise": "yellow",  # 新增
            "flow_stiction": "darkgreen"  # 新增
        }
        ...
        ax2.set_title(
            f'控制曲线（含扰动与参数更新 - PID模式: {Config.PID_MODES[pid_mode]}, 系统模式: {Config.MODES[system_mode]}, 控制类型: {control_type})')
        ax2.set_xlabel('时间 (s)')
        ax2.set_ylabel(f'响应值 ({"流量" if control_type == "flow" else "温度"})')
        ax2.legend(loc='upper right', fontsize=8)
        ax2.grid(True, alpha=0.3)

        # 创建真实仿真数据（无控制器影响的系统自然响应）
        if control_type == "flow":
            true_system = FlowSystem(**true_params, mode=system_mode)
        else:
        ...
        for i, time in enumerate(t):
            ...
            valve = u[i]  # 使用实际的输入信号
            if control_type == "flow":
                var = true_system.update(valve, time, disturbances=disturbances)
            else:
                var = true_system.update(valve, time, disturbances=disturbances)
            true_data.append(var)

        # 绘制真实仿真曲线
        true_data_line, = ax2.plot(t, true_data, 'c--', label='真实仿真值（无控制）', linewidth=1.5, alpha=0.7)
        data_line, = ax2.plot([], [], 'b-', label='实际响应值（有控制）', linewidth=1.5)
        ...
        ax5.set_title(
            f'PID参数变化趋势 (PID模式: {Config.PID_MODES[pid_mode]}, 系统模式: {Config.MODES[system_mode]}, 控制类型: {control_type})')
        ax5.set_xlabel('时间 (s)')
        ...
        self.update_params_text(params_text, system_params, pid_params, lambda_val=system_params['T'],
                                pid_mode=pid_mode, system_mode=system_mode, control_type=control_type)
        ...
        self.plot_data = {
            ...
            'lines': (true_data_line, data_line, valve_line, error_line, kp_line, ti_line, td_line),
            'text': params_text,
    ...
    def update_params_text(text_obj, system_params, pid_params, lambda_val, update_count=0, initial_params=None,
                           pid_mode="standard", system_mode="standard", control_type="temperature"):
        """更新参数显示文本"""
        K, T, L = system_params['K'], system_params['T'], system_params['L']
...
{initial_text}当前PID参数 {status}(λ={lambda_val:.1f}, PID模式:{Config.PID_MODES[pid_mode]}, 系统模式:{Config.MODES[system_mode]}, 控制类型:{control_type}):
    比例增益 Kp = {Kp:.2f}
    积分时间 Ti = {Ti:.1f} s
    微分时间 Td = {Td:.1f} s

目标值: {Config.TARGET_FLOW if control_type == "flow" else Config.TARGET_TEMPERATURE} | 初始值: {Config.INIT_FLOW if control_type == "flow" else Config.INIT_TEMPERATURE}"""
        text_obj.set_text(text)

    def update_plots(self, time_data, data, true_data, valve_data, error_data, kp_data, ti_data, td_data,
                     update_lines):
        """实时更新绘图数据"""
        (true_data_line, data_line, valve_line, error_line, kp_line, ti_line, td_line) = self.plot_data['lines']
        (ax2, _, _, _) = self.plot_data['axes']
        _, _, update_marker = self.plot_data['markers']

        # 更新曲线数据
        data_line.set_data(time_data, data)
        true_data_line.set_data(time_data, true_data[-len(time_data):])  # 确保数据长度一致
        ...
        update_marker.set_data(update_times, [
            Config.TARGET_FLOW if self.plot_data['control_type'] == "flow" else Config.TARGET_TEMPERATURE + 8] * len(
            update_times))

        # 更新标题
        current_time = time_data[-1] if time_data else 0
        ax2.set_title(f'控制曲线（当前时间：{current_time:.0f}s）')
...
class ControlMonitor:
    ...
    def run(self):
        ...
        # 选择控制类型（温度或流量）
        control_type = self.data_handler.choose_control_type()
        self.logger.info(f"选择控制类型: {control_type}")
        ...
        if control_type == "flow":
            ...
            system = FlowSystem(**initial_true_params, mode=system_mode)
            setpoint = Config.TARGET_FLOW
            init_value = Config.INIT_FLOW
        else:
            ...
            init_value = Config.INIT_TEMPERATURE

        # 生成仿真数据（如未提供）
        if response_data is None:
            if control_type == "flow":
                u = system.generate_flow_input(t)
                response_data = system.simulate_with_input(t, u, disturbances)
            else:
                u = system.generate_non_step_input(t)
                response_data = system.simulate_with_input(t, u, disturbances)
            # 应用低通滤波到仿真生成的数据
            response_data = self.data_handler.apply_low_pass_filter(response_data)
            self.logger.info(
                f"对仿真生成的数据应用了低通滤波 (降噪强度: {noise_reduction_level}, 截止频率: {Config.NOISE_REDUCTION_LEVELS[noise_reduction_level]['cutoff_freq']})")
            ...
            # 保存生成的数据
            save_path = os.path.join(Config.DATA_SAVE_DIR,
                                     f"{'flow' if control_type == 'flow' else 'temperature'}_response_data.csv")
            pd.DataFrame({
                "时间(s)": t,
                "输入信号(%)": u,
                "动态响应值": response_data
        ...
        # 启动实时监控
        print(
            f"\n启动{'流量' if control_type == 'flow' else '温度'}控制监控 (系统模式: {Config.MODES[system_mode]}, PID模式: {Config.PID_MODES[pid_mode]})...")
        self._run_real_time_monitor(t, u, setpoint, control_type, true_params=initial_true_params,
    ...
    def _run_real_time_monitor(self, t, u, setpoint, control_type, true_params, identified_params, response_data,
                               disturbances,
        ...
        fig, plot_data = self.visualizer.init_real_time_fig(t, u, setpoint, identified_params,
                                                            ...
                                                            disturbances, pid_mode, system_mode, control_type)
        plot_data['control_type'] = control_type  # 保存控制类型用于绘图更新
        ...
        true_data_line, data_line, valve_line, error_line, kp_line, ti_line, td_line = plot_data['lines']
        ...
        if control_type == "flow":
            init_Kp, init_Ti, init_Td = self.identifier.lambda_tuning_for_flow(K, T, mode=system_mode)
        else:
        ...
        # 优化：流量设定值管理器
        if control_type == "flow":
            sp_manager = FlowSetpointManager(ramp_rate=0.8)

        current_var = Config.INIT_FLOW if control_type == "flow" else Config.INIT_TEMPERATURE
        ...
        if control_type == "flow":
            ...
            true_system = TemperatureSystem(**true_params, mode=system_mode)
        true_current_var = Config.INIT_FLOW if control_type == "flow" else Config.INIT_TEMPERATURE
        ...
        print(f"\n" + "=" * 50)
        print(
            f"系统运行模式: {Config.MODES[system_mode]} | PID控制模式: {Config.PID_MODES[pid_mode]} | 控制类型: {control_type}")
        print("初始系统参数辨识结果：")
        ...
        def on_slider_change(val):
            nonlocal K, T, L
            if control_type == "flow":
                new_Kp, new_Ti, new_Td = self.identifier.lambda_tuning_for_flow(K, T, mode=system_mode)
            else:
        ...
        try:
            for i, time in enumerate(t):
                # 优化：流量控制时使用设定值斜坡
                if control_type == "flow":
                    smooth_setpoint = sp_manager.update(setpoint, 1.0)  # dt = 1.0
                    valve_opening = pid.compute(smooth_setpoint, current_var)
                else:
                ...
                if control_type == "flow":
                    true_current_var = true_system.update(u[i], time, disturbances=disturbances)  # 使用原始输入信号
                else:
                    true_current_var = true_system.update(u[i], time, disturbances=disturbances)

                # 记录数据
                time_data.append(time)
                data.append(current_var)
                true_data.append(true_current_var)
                ...
                # 检测初始稳态（达到目标值后开启整定）
                if stabilization_time is None and state_analyzer.is_stable(data, setpoint):
                ...
                if tuning_enabled and not in_pid_tuning_phase:
                    ...
                    is_disturbance, disturbance_desc = state_analyzer.detect_instability_optimized(
                        data, setpoint, valve_data, time_data
                    )
                    ...
                    if is_disturbance:
                        ...
                        if current_disturbance:
                            ...
                            if not current_disturbance['updated'] and (time - current_disturbance["time"]) > 50:
                                ...
                                window_y = np.array(data[window_start:])
                                ...
                                if control_type == "flow":
                                    ...
                                    new_Kp, new_Ti, new_Td = self.identifier.lambda_tuning_for_flow(K, T,
                                                                                                    mode=system_mode)
                                else:
                                ...
                                elif current_disturbance["type"] == "flow_noise":
                                    # 流量噪声：降低微分项
                                    new_Td = 0.0
                                    print(f"🔧 流量噪声优化：关闭微分项 → Td: {new_Td:.1f}s")
                                elif current_disturbance["type"] == "flow_stiction":
                                    # 流量卡涩：降低比例增益，增加积分时间
                                    new_Kp *= 1.2
                                    new_Ti *= 1.2
                                    new_Td = 0.0
                                    print(
                                        f"🔧 流量卡涩优化：Kp提高20%，Ti提高20%，关闭微分 → {new_Kp:.2f}, {new_Ti:.1f}s, {new_Td:.1f}s")
                ...
                for d in disturbances:
                    if (d['updated'] and not recovery_status[d["type"]] and
                        ...
                        if state_analyzer.is_stable(data, setpoint):
                            recovery_status[d["type"]] = True
                            print(f"🎉 {d['description']}已恢复稳定（{time:.0f}s），当前值: {current_var:.1f}")
                            self.logger.info(
                                f"扰动 {d['description']} 已恢复稳定 - 时间: {time:.0f}s, 值: {current_var:.1f}")
                ...
                if i % 50 == 0:
                    ...
                    print(
                        f"时间: {time:.0f}s | 系统模式: {Config.MODES[system_mode]} | PID模式: {Config.PID_MODES[pid_mode]} | 控制类型: {control_type} | 实际响应值: {current_var:.1f} | 真实仿真值: {true_current_var:.1f} | 误差: {error:.2f} | "
                        f"阀门: {valve_opening:.1f}% | 状态: {stable_status} | 更新: {params_update_count}次 | {tuning_phase_status}")

                # 批量更新绘图
                if i % Config.PLOT_REFRESH_INTERVAL == 0:
                    self.visualizer.update_plots(time_data, data, true_data, valve_data, error_data,
                                                 kp_data, ti_data, td_data, update_lines)

            # 保存数据
            real_time_data = np.column_stack(
                (time_data, data, true_data[-len(time_data):], valve_data, error_data,
                 kp_data, ti_data, td_data))
            self.data_handler.save_data(real_time_data,
                                        ["时间(s)", f"实际响应{'流量' if control_type == 'flow' else '温度'}",
                                         f"真实仿真{'流量' if control_type == 'flow' else '温度'}", "阀门开度(%)",
                                         "误差",
                                         "Kp", "Ti/10", "Td*2"],
                                        f"real_time_{'flow' if control_type == 'flow' else 'temperature'}_control_data.csv")
            ...
            print(
                f"系统运行模式: {Config.MODES[system_mode]} | PID控制模式: {Config.PID_MODES[pid_mode]} | 控制类型: {control_type}")
            print(
                f"初始值：{Config.INIT_FLOW if control_type == 'flow' else Config.INIT_TEMPERATURE} | 目标：{Config.TARGET_FLOW if control_type == 'flow' else Config.TARGET_TEMPERATURE}")
            print(
                f"最终实际响应值：{current_var:.1f} | 最终真实仿真值：{true_current_var:.1f} | 最终误差：{error:.2f}")
            print(f"初始有效参数 → 最终参数：")
            ...
            self.logger.info(f"最终值: {current_var:.1f}, 误差: {error:.2f}, 更新次数: {params_update_count}")
            self.logger.info(f"最终PID参数: Kp={pid.Kp:.2f}, Ti={pid.Ti:.1f}s, Td={pid.Td:.1f}s")
            self.logger.info(
                f"系统模式: {Config.MODES[system_mode]}, PID模式: {Config.PID_MODES[pid_mode]}, 控制类型: {control_type}")
            ...
            if len(time_data) > 0:
                ...
                self.data_handler.save_data(real_time_data,
                                            ["时间(s)", f"实际响应{'流量' if control_type == 'flow' else '温度'}",
                                             f"真实仿真{'流量' if control_type == 'flow' else '温度'}", "阀门开度(%)",
                                             "误差"],
                                            f"real_time_{'flow' if control_type == 'flow' else 'temperature'}_control_data_interrupted.csv")
            print("\n" + "=" * 50)
            print(
                f"系统运行模式: {Config.MODES[system_mode]} | PID控制模式: {Config.PID_MODES[pid_mode]} | 控制类型: {control_type}")
            print("监控手动终止")
            print(
                f"当前实际响应值：{current_var:.1f} | 当前真实仿真值：{true_current_var:.1f} | 误差：{error:.2f}")
            print(f"初始有效参数 → 当前参数：")
            print(f"Kp: {initial_valid_params[0]:.2f} → {pid.Kp:.2f}")
            print(f"Ti: {initial_valid_params[1]:.1f}s → {pid.Ti:.1f}s")
            print(f"Td: {initial_valid_params[2]:.1f}s → {pid.Td:.1f}s")
            ...
            self.logger.info(f"中断时值: {current_var:.1f}, 误差: {error:.2f}, 更新次数: {params_update_count}")

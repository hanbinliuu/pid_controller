...
class Config:
    ...
    # 仿真与存储参数
    # SIMULATION_DURATION = 10000  # 仿真总时长(s) - 已移除
...
class DisturbanceGenerator:
    ...
    def generate_random_disturbances():
        ...
        # 随机选择扰动数量 (2-4个)
        # num_disturbances = min(np.random.randint(3, 6), 8)  # 最多4种失效场景
...
class ControlMonitor:
    ...
    def run(self):
        ...
        K, T, L = self.identifier.identify_fopdt(t if len(t) > 0 else np.arange(len(response_data)), response_data, u)
    ...
    def _run_real_time_monitor(self, u, setpoint, true_params, identified_params, response_data, disturbances,
        ...
        try:
            while True:  # 无限运行主循环
                ...
                # 计算真实系统温度（无控制器影响）
                # 使用动态扩展的输入信号
                current_input = u[min(step, len(u) - 1)] if len(u) > 0 else 50.0  # 默认值
                true_current_temp = true_system.update(current_input, time, disturbances=disturbances)
                ...
                if tuning_enabled:
                    ...
                    if all(recovery_status.get(d["type"], True) for d in disturbances):
                        # 随机决定是否生成新扰动（每1000步有50%概率生成新扰动）
                        if time % 1000 == 0 and np.random.random() > 0.5:
                            # 生成新的扰动
                ...
                if step % 50 == 0:
                    ...
                    print(
                        ...
                        f"阀门: {valve_opening:.1f}% | 状态: {stable_status} | 更新: {params_update_count}次 | {tuning_phase_status} | 扰动总数: {len(disturbances)}")
                ...
                if step >= len(u):
                    ...
                    new_u = true_system.generate_non_step_input(new_time)
                    u = np.append(u, new_u)
                ...
                if step % 10000 == 0:
                    ...
                    real_time_data = np.column_stack(
                        (time_data[-10000:], temp_data[-10000:], true_temp_data[-10000:],
                    ...
                    np.savetxt(
                        ...
                        real_time_data,
                        delimiter=",",
                        header="时间(s),实际响应温度(℃),真实仿真温度(℃),阀门开度(%),误差(℃)",
                        comments=""
                    )
                    self.logger.info(f"数据已保存至：{save_path}")

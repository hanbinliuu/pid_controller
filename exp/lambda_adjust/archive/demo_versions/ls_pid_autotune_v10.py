...
class DisturbanceGenerator:
    ...
    def generate_random_disturbances():
        ...
        # 随机选择扰动数量 (2-4个)
        num_disturbances = min(5, 10)  # 最多5种失效场景（包括新增的流量扰动）
...
class DataHandler:
    ...
    def choose_mode(self, using_json_data=False):
        ...
        if using_json_data:
                ...
                if choice == "1":
                    ...
                    while True:
                        ...
                        if flow_choice in ["1", "2", "3"]:
                            flow_modes = [Mode.STANDARD, Mode.ANTI_DISTURBANCE, Mode.ANTI_NOISE]
                            selected_mode = flow_modes[int(flow_choice) - 1]
                            # 将选择的模式转换为流量控制模式
                            if selected_mode == Mode.STANDARD:
                                return Mode.FLOW_CONTROL
                            elif selected_mode == Mode.ANTI_DISTURBANCE:
                                return Mode.FLOW_CONTROL
                            elif selected_mode == Mode.ANTI_NOISE:
                                return Mode.FLOW_CONTROL
                        else:
                            print("❌ 输入错误，请选择1-3")
                else:
            ...
            print(f"\n=== 选择运行模式 ===")
            for i, (key, desc) in enumerate(Config.MODES.items(), 1):
                if key != Mode.FLOW_CONTROL:  # 仿真模式下不显示流量控制模式
                    print(f"{i}. {desc} ({key})")
...
class SystemIdentifier:
    ...
    def auto_tune_from_json(self, t, temp_data, setpoint, tuning_method, mode=Mode.STANDARD):
        ...
        try:
            if mode == Mode.FLOW_CONTROL:
                # 流量模式使用一阶模型
                K, T = self.identify_first_order(t_seg_rel, y_seg, setpoint)
                L = 0.0  # 假设无滞后
                print(f"🔍 一阶模型辨识结果: K={K:.3f}, T={T:.3f}")
            else:
                K, T, L = self.identify_fopdt(t_seg_rel, y_seg, setpoint)
...
目标温度: {Config.TARGET_TEMPERATURE if system_mode != Mode.FLOW_CONTROL else 50.0}℃ | 初始温度: {Config.INIT_TEMPERATURE if system_mode != Mode.FLOW_CONTROL else 0.0}℃"""
...
class ControlMonitor:
    ...
    def run(self):
        ...
        # 选择控制模式 - 优化：只有在使用JSON数据时才显示流量控制选项
            ...
            if len(pb_data) > 0 and len(ti_data) > 0 and len(td_data) > 0:
                ...
                if new_params:
                    ...
                    if system_mode == Mode.FLOW_CONTROL:
                        ...
                        init_pb, init_ti, init_td = self._get_initial_params_for_flow(K, T, tuning_method, system_mode)
                    else:
        ...
        # 初始参数辨识
        print(
            f"\n全局初始值：{Config.INIT_TEMPERATURE if system_mode != Mode.FLOW_CONTROL else 0.0}℃ | 目标值：{Config.TARGET_TEMPERATURE if system_mode != Mode.FLOW_CONTROL else 50.0}")
        ...
        self._run_real_time_monitor(u, setpoint=Config.TARGET_TEMPERATURE if system_mode != Mode.FLOW_CONTROL else 50.0,
    ...
    def _get_initial_params_for_flow(self, K, T, tuning_method, system_mode):
        """根据整定方法获取流量控制初始PID参数"""
        if tuning_method == TuningMethod.LAMBDA:
            return self.identifier.lambda_tuning_for_flow(K, T, mode=system_mode)
        elif tuning_method == TuningMethod.COHEN_COON:
            # 对于流量控制，Cohen-Coon可能不太适用，使用Lambda方法
            return self.identifier.lambda_tuning_for_flow(K, T, mode=system_mode)
        else:
            # 默认使用Lambda方法
            return self.identifier.lambda_tuning_for_flow(K, T, mode=system_mode)
                        ...
                        if current_disturbance:
                            ...
                            if not current_disturbance['updated'] and (time - current_disturbance["time"]) > 50:
                                ...
                                if system_mode == Mode.FLOW_CONTROL:
                                    ...
                                    new_pb, new_ti, new_td = self.identifier.lambda_tuning_for_flow(K, T,
                                                                                                    mode=system_mode)
                                else:
                                    ...
                                    K, T, L = new_K, new_T, new_L
                                    lambda_val = lambda_slider.val
                                    new_pb, new_ti, new_td = self._get_updated_params(K, T, L, lambda_val,
                                                                                      tuning_method,
                                                                                      system_mode)
    ...
    def _adjust_pid_for_disturbance(self, disturbance_type, pb, ti, td):
        ...
        elif disturbance_type == DisturbanceType.FLOW_NOISE:
            # 流量噪声：降低微分项
            td = 0.0
            print(f"🔧 流量噪声优化：关闭微分项 → Td: {td:.1f}s")
        elif disturbance_type == DisturbanceType.FLOW_STICTION:
            # 流量卡涩：降低比例增益，增加积分时间
            pb *= 1.2
            ti *= 1.2
            td = 0.0
            print(f"🔧 流量卡涩优化：Pb提高20%，Ti提高20%，关闭微分 → {pb:.2f}%, {ti:.1f}s, {td:.1f}s")

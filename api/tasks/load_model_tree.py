...
DEFAULT_POINT_PATH = Config.BFF_MODEL_POINT_PATH  # 默认测点路径


def load_model_tree_and_sync() -> Dict[str, Any]:
    """
    加载模型树并同步到数据库
...
def _sync_loop_to_db(db, instance: Dict[str, Any]) -> str:
    ...
    # 查询是否已存在
    existing_loop = LoopInfoDAO.get_by_loop_uri(db, loop_uri)
    
    # 提取实例信息
    loop_name = instance.get('displayName', instance.get('browseName', ''))
    loop_path = instance.get('uriPath', '')
